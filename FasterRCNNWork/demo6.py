...
class UrbanFlows():
    ...
    def detectCars(self, im, im_copy, frame_gray, net, tracks):
        print "detect cars running"
        ...
        for cls_ind, cls in enumerate(CLASSES[1:]):
            ...
            if cls == "car":
                ...
                for i in inds:
                    ...
                    w = bbox[2] - bbox[0]
                    h = bbox[3] - bbox[1]
                    cv2.rectangle(im_copy, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (255, 0, 0), 3)
                    #rectangleTracks.append([bbox[0], bbox[1], bbox[2], bbox[3]])
                    
                    car_centroid = [(bbox[0]+(w/2)), (bbox[1]+(h/2))]
                    cv2.circle (im_copy, ( (bbox[0]+(w/2)) , (bbox[1]+(h/2)) ), 4, (255, 0, 0), 4)
    ...
    def demo(self, net):
        ...
        #     frame_gray = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
        #     im_copy = im.copy()
        #     c.detectCars(im, im_copy, frame_gray, net, rectangleTracks, tracks)
